...
namespace RaidForge.Commands
{
    ...
    {
        [Command("raidforge", "RaidForge admin tools. Usage: .raidforge ?, .raidforge status, .raidforge cache", adminOnly: true)]
        public void RaidForgeCommand(ChatCommandContext ctx, string action = null)
        {
            try
            {
                ...
                switch (action)
                {
                    ...
                    default:
                        ctx.Reply(ChatColors.ErrorText($"Unknown RaidForge action '{action}'. Use '.raidforge ?' for options."));
                        break;
                }
            }
            catch (Exception ex)
        ...
        private static void ShowHelp(ChatCommandContext ctx)
        {
            ...
            ctx.Reply(ChatColors.InfoText(".raidforge status") + ChatColors.MutedText(" - Shows current RaidForge system status and key counts."));
            ctx.Reply(ChatColors.InfoText(".raidforge cache") + ChatColors.MutedText(" - Shows cache/world count details."));
            ctx.Reply(ChatColors.InfoText(".raidrefreshcache") + ChatColors.MutedText(" - Rebuilds ownership cache."));
            ctx.Reply(ChatColors.InfoText(".raidauto") + ChatColors.MutedText(" - Clears manual raid override and resumes the schedule."));
            ctx.Reply(ChatColors.InfoText(".raidconfigview ?") + ChatColors.MutedText(" - Shows numbered config sections."));
            ctx.Reply(ChatColors.InfoText(".raidconfigviewall") + ChatColors.MutedText(" - Shows all loaded config sections."));
        ...
        private static void ShowCacheCounts(ChatCommandContext ctx)
        {
            ...
            ctx.Reply(ChatColors.InfoText("Tip: use ") + ChatColors.AccentText(".raidrefreshcache") + ChatColors.InfoText(" to rebuild ownership cache."));
