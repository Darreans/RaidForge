...
namespace RaidForge.Commands
{
	public class RaidCommands
	{
		...
		public void RemoveOrp(ChatCommandContext ctx, string playerName)
		{
			...
			if (!UserHelper.FindUserEntity(em, playerName, out Entity targetUserEntity, out User targetUserData, out string foundName))
			{
			...
			if (targetClanEntity.Exists() && em.HasComponent<ClanTeam>(targetClanEntity))
			{
				persistentKey = PersistentKeyHelper.GetClanKey(em, targetClanEntity);
				contextualName = em.GetComponentData<ClanTeam>(targetClanEntity).Name.ToString() + " (Clan)";
			}
			else
			{
				persistentKey = PersistentKeyHelper.GetUserKey(targetUserData.PlatformId);
				contextualName = foundName;
			}

			if (OfflineGraceService.AdminForceRemoveProtection(persistentKey))
			{
		...
		public void RaidTime(ChatCommandContext ctx)
		{
			...
			if (nextRaidStart != DateTime.MaxValue)
			{
				...
				ctx.Reply(ChatColors.InfoText($"{nextRaidStart:f}"));
			}
					...
					if (OfflineRaidProtectionConfig.EnableOfflineRaidProtection.Value)
					{
						if (OfflineGraceService.TryGetOfflineStartTime(persistentKey, out DateTime offlineStartTime))
						{
							...
							if (graceMinutes > 0 && timeOffline.TotalMinutes < graceMinutes)
							{
								...
								ctx.Reply($"{ChatColors.InfoText(baseName)}: {ChatColors.WarningText("RAIDABLE")} {ChatColors.InfoText("Vulnerability Period:")} {ChatColors.WarningText(timeStr)} {ChatColors.InfoText("remaining")}");
							}
