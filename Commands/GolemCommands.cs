...
namespace RaidForge.Commands
{
	public class GolemCommands
	{
		...
		public void GolemTransformCommand(ChatCommandContext ctx, string playerName = null)
		{
			...
			try
			{
				ApplyBuff(targetCharacterEntity, PrefabData.SiegeGolemBuff);
				ctx.Reply(ChatColors.SuccessText($"Applied Siege Golem form to {targetName}."));
			}
			catch (Exception ex)
			{
				ctx.Reply(ChatColors.ErrorText($"Failed to apply golem buff: {ex.Message}"));
				RaidForge.Plugin.Logger.LogError(ex);
			}
		}
		...
		private static void ApplyBuff(Entity target, Stunlock.Core.PrefabGUID buffGuid)
		{
			...
			var fromCharacter = new FromCharacter { Character = target };

			if (entityManager.HasComponent<PlayerCharacter>(target))
			{
				var pc = entityManager.GetComponentData<PlayerCharacter>(target);
				if (pc.UserEntity != Entity.Null)
				{
					fromCharacter.User = pc.UserEntity;
				}
			}
