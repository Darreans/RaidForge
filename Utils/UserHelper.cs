...
namespace RaidForge.Utils
{
    public static class UserHelper
    {
        ...
        public static bool TryGetSoulShardsOnPerson(EntityManager em, Entity characterEntity, out HashSet<PrefabGUID> foundShards, out string locationSummary)
        {
            ...
            try
            {
                if (InventoryUtilities.TryGetInventoryEntities(em, characterEntity, ref inventoryEntities))
                {
                    for (int i = 0; i < inventoryEntities.Length; i++)
                    {
                        Entity inventoryEntity = inventoryEntities[i];
                        if (!em.Exists(inventoryEntity) || !em.HasBuffer<InventoryBuffer>(inventoryEntity))
                        {
                            continue;
                        }
                        ...
                        foreach (var item in inventory)
                        {
                            if (PrefabData.SoulShardPrefabGUIDs.Contains(item.ItemType))
                            {
                                ...
                                em.TryGetComponentData<PrefabGUID>(item.ItemEntity._Entity, out var prefabGuid) &&
                                PrefabData.SoulShardPrefabGUIDs.Contains(prefabGuid))
                            {
            ...
            {
                if (inventoryEntities.IsCreated) inventoryEntities.Dispose();
            }
        ...
        public static bool TryGetSoulShardsInOwnedPedestals(EntityManager em, Entity userEntity, out HashSet<PrefabGUID> foundShards, out string reason)
        {
            ...
            var pedestalGuids = PrefabData.SoulShardPedestalPrefabGUIDs;
        ...
        public static bool FindUserEntity(EntityManager em, string identifier, out Entity userEntity, out User userData, out string characterName)
        {
            ...
            try
            {
                ...
                allUserEntities = userQuery.ToEntityArray(Allocator.Temp);
